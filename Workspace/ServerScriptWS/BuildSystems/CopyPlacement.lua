	...
	if plot.wazne.Owner.Value == plr.Name then
		...
		for i,n in pairs(plot.PlacedObjects:GetChildren()) do
			if n:GetAttribute("beingCopied") == true then
				...
				item = n:Clone()
				task.wait()
				item:SetAttribute("beingCopied",nil)
		...
		if plr.leaderstats.Cash.Value < item.Price.Value then
			item:Destroy()
			game.ReplicatedStorage.Events.SystemsEvents.shutdownSystems:FireClient(plr)
		...
		if obj.Parent.Parent.wazne.Owner.Value == plr.Name then
			...
			plr.leaderstats.Cash.Value -= item.Price.Value

			if string.match(item.Name,"Shelf") or item.Name == "DisplayTable" then
				item.Towar.IleArtykul.Value = 0
				item.Towar.KtoryArtykul.Value = ""
			end
...
game.ReplicatedStorage.Events.CopySet.OnServerEvent:Connect(function(plr, model)
	local plot = plotManager.returnPlot(workspace.Plots, plr)
	if plot.wazne.Owner.Value == plr.Name then
		if model:IsA("Model") and model.PrimaryPart and model.Parent.Parent.wazne.Owner.Value == plr.Name then
			model:SetAttribute("beingCopied",true)
		end
	end
end)

game.ReplicatedStorage.Events.CopyCancel.OnServerEvent:Connect(function(plr)
	local plot = plotManager.returnPlot(workspace.Plots, plr)
	if plot.wazne.Owner.Value == plr.Name then
		for i,n in pairs(plot.PlacedObjects:GetChildren()) do
			n:SetAttribute("beingCopied",nil)
		end
	end
end)


game.ReplicatedStorage.Remotes.copy.OnServerInvoke = place